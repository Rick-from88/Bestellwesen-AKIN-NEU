...
import { createBestellung, listBestellungen, BestellungStatus } from './repositories/bestellungen';
import { createLieferant, listLieferanten } from './repositories/lieferanten';
...
app.post('/api/bestellungen', async (req, res) => {
    ...
    const bestellDatum = typeof req.body.bestellDatum === 'string' ? req.body.bestellDatum : undefined;

    if (!artikelId || !lieferantId || !menge || menge <= 0) {
        res.status(400).json({ error: 'artikelId, lieferantId und menge sind Pflichtfelder.' });
        return;
    }

    try {
        const bestellung = await createBestellung({ artikelId, lieferantId, menge, status, bestellDatum });
...
app.get('/api/lieferanten', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Lieferanten konnten nicht geladen werden.' });
    }
});

app.post('/api/lieferanten', async (req, res) => {
    const name = typeof req.body.name === 'string' ? req.body.name.trim() : '';
    const kontaktPerson = typeof req.body.kontaktPerson === 'string' ? req.body.kontaktPerson.trim() : undefined;
    ...
    try {
        const lieferant = await createLieferant({ name, kontaktPerson, email, telefon });
...
app.post('/api/artikel', async (req, res) => {
    const name = typeof req.body.name === 'string' ? req.body.name.trim() : '';
    const beschreibung = typeof req.body.beschreibung === 'string' ? req.body.beschreibung.trim() : undefined;
    const preis = parseNumber(req.body.preis);
    const lagerbestand = parseInteger(req.body.lagerbestand);

    if (!name || preis === null || preis < 0 || lagerbestand === null || lagerbestand < 0) {
        res.status(400).json({ error: 'name, preis und lagerbestand sind Pflichtfelder.' });
        return;
    }

    try {
        const artikel = await createArtikel({ name, beschreibung, preis, lagerbestand });
