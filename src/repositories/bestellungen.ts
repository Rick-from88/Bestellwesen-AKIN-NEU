...
export const getNextBestellnummer = async (dateIso?: string): Promise<number> => {
    ...
    try {
        ...
        const maxRes = await client.query('select max(bestellnummer) as mx from bestellungen where bestellnummer between $1 and $2', [lower, upper]);
...
export const updateBestellung = async (
    ...
    try {
        ...
        const nrRes = await client.query('select bestellnummer from bestellungen where id = $1', [id]);
