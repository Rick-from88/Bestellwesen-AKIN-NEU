...
const testSmtpConnection = (override) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (override && (override.host || override.port || override.user || override.pass)) {
        const t = (0, exports.createTransporter)({ host: override.host, port: override.port, user: override.user, pass: override.pass });
        try {
            yield t.verify();
            return { ok: true, used: { host: override.host || MAIL_HOST, port: Number((_a = override.port) !== null && _a !== void 0 ? _a : MAIL_PORT), user: override.user || MAIL_USER, from: override.from || MAIL_FROM } };
        }
        catch (err) {
            const e = err;
            return { ok: false, error: (e && (e.message || e)), used: { host: override.host || MAIL_HOST, port: Number((_b = override.port) !== null && _b !== void 0 ? _b : MAIL_PORT), user: override.user || MAIL_USER, from: override.from || MAIL_FROM } };
        }
    }
    try {
        yield transporter.verify();
        return { ok: true, used: { host: MAIL_HOST, port: MAIL_PORT, user: MAIL_USER, from: MAIL_FROM } };
    }
    catch (err) {
        const e = err;
        return { ok: false, error: (e && (e.message || e)), used: { host: MAIL_HOST, port: MAIL_PORT, user: MAIL_USER, from: MAIL_FROM } };
    }
});
