  ...
  try {
    ...
    for (const s of suppliers) {
      const r = await client.query(
        ...
        [s.name, s.kontakt, s.email, s.telefon, s.strasse, s.plz, s.stadt, s.land],
      );
    ...
    const articles = [
      { name: 'Artikel Alpha', beschreibung: 'Testartikel Alpha', preis: 12.5, lager: 100, minBest: 5 },
      { name: 'Artikel Beta', beschreibung: 'Testartikel Beta', preis: 7.9, lager: 50, minBest: 2 },
    ];
    ...
    await client.query(
      ...
      [JSON.stringify([{ id: 1, text: 'Testnotiz', createdAt: new Date().toISOString(), done: false }])],
    );
